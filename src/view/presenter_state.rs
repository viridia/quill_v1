...
impl ViewHandle {
    ...
    pub fn new<
        ...
        F: FnMut(Cx<Props>) -> V + Send + 'static,
    >(
        presenter: F,
        props: Props,
    ) -> Self {
...
pub struct PresenterState<V: View, Props: Send, F: FnMut(Cx<Props>) -> V> {
...
impl<V: View, Props: Send + PartialEq, F: FnMut(Cx<Props>) -> V> PresenterState<V, Props, F> {
...
impl<V: View, Props: Send + Clone + PartialEq + 'static, F: FnMut(Cx<Props>) -> V + Send>
